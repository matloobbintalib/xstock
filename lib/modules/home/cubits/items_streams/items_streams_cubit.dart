...
import 'package:xstock/modules/home/cubits/items_streams/items_streams_state.dart';
import 'package:xstock/modules/home/models/group_model.dart';

import '../../../authentication/models/user_model.dart';

class ItemsStreamsCubit extends Cubit<ItemsStreamsState> {
  ItemsStreamsCubit() : super(ItemsStreamsState.initial());
  UserAccountRepository userAccountRepository = sl<UserAccountRepository>();

  void getItemsStreams(String groupId) async {
    ...
    try {
      ...
      CollectionReference groupCollection = await userRef.collection(Endpoints.groupsTable);
      CollectionReference itemsCollection = await groupCollection.doc(groupId).collection(Endpoints.itemsTable);
      emit(state.copyWith(itemsStreams: itemsCollection.snapshots(), hasData: true));
    } catch (e) {
      emit(state.copyWith(hasData: false));
    }
  }
