...
class AddItemPage extends StatelessWidget {
  ...
  const AddItemPage(
      {super.key, required this.groupId, required this.userModel});
...
class _AddItemPageViewState extends State<AddItemPageView> {
  ...
  void addItem(String colorCode, String userId) async {
    ToastLoader.show();
    DocumentReference groupRef = groupsCollection.doc(groupId);
    CollectionReference itemsCollection =
        groupRef.collection(Endpoints.itemsTable);
    String randomString = generateRandomString(10);
    DateTime now = DateTime.now();
    Timestamp timestamp = Timestamp.fromDate(now);
    String itemRef = '${randomString}${timestamp.millisecondsSinceEpoch}';
    ...
    await outerItemsCollection.doc(itemRef).set(itemModel.toMap());
                      ...
                      if (itemNameController.text.isNotEmpty) {
                        if (context
                          ...
                          if (groupId.isNotEmpty) {
                            addItem(color.value.toString(), user.id.toString());
                          } else {
